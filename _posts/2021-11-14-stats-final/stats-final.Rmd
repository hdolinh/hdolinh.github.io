...
**TRIED TO DO SOME TIME SERIES ANALYSIS**

```{r}
wells_subset <- wells_region %>% 
  select(well_id, date_completed) %>% 
  group_by(well_id, date_completed) %>% 
  count()

wells_tsbl <- as_tsibble(wells_subset, key = well_id)

ggplot(data = wells_tsbl, aes(x = date_completed, y = n)) +
  geom_line()
```
